...
Inductive prefix : Type :=
 | Inn : name -> name -> prefix
 | Out : name -> name -> prefix
 | Tau : prefix. 
...
Notation "a * [ b ] P" := (pi (Inn a b) P)    (at level 45).
Notation "a ^ [ b ] P" := (pi (Out a b) P)    (at level 44).
Definition t (P : proc) := pi Tau P.

Notation "P + Q" := (plus P Q).
Notation "P / Q" := (par P Q).
Notation "0" := zero.
Notation "! P" := (rep P)                     (at level 46).
...
Lemma foldpar_distr_par : forall l1 l2, 
(foldpar l1) / foldpar (l2) = foldpar (l1 ++ l2).
...
Lemma foldpar_comm : forall l1 l2,
foldpar (l1 ++ l2) = foldpar (l2 ++ l1).
Proof. intros. rewrite <- foldpar_distr_par. rewrite comm_par.
apply foldpar_distr_par.
Qed.

Lemma foldpar_mult : forall l1 l2 lr1 lr2,
 ((foldpar (l1 ++ lr1)) / (foldpar (l2 ++ lr2))) = 
 foldpar ((l1 ++ l2) ++ lr1 ++ lr2).
Proof. intros. rewrite foldpar_distr_par.
rewrite app_assoc. rewrite <- foldpar_distr_par.
rewrite foldpar_comm. rewrite foldpar_comm. rewrite <- app_assoc.
rewrite <- foldpar_distr_par. rewrite foldpar_comm.
rewrite foldpar_distr_par. rewrite foldpar_distr_par.
...
Lemma wrong : forall l1 l2 lr1 lr2, 
foldpar (map sigma l1 ++ map rep lr1) + foldpar (map sigma l2 ++ map rep lr2)
 = foldpar (map sigma (l1 ++ l2) ++ map rep (lr1 ++ lr2)).
Proof. Admitted.
...
Theorem normal : forall (P : proc),
  (exists ns l lrep,
    P = new ns (foldpar (map sigma l ++ map rep lrep))
  ).
Proof. intros. induction P.
 - exists []. exists [[(p, P)]]. exists []. rewrite app_nil_r.
   rewrite <- auto_gen_new. (* P = new a P *)
   unfoldall.
   rewrite unit_sum. rewrite unit_par.
   reflexivity.
 - destruct IHP1 as [ns1 [l1 [lr1 H1]]].
   destruct IHP2 as [ns2 [l2 [lr2 H2]]].
   exists (ns1 ++ ns2). exists (l1 ++ l2). exists (lr1 ++ lr2).
   rewrite <- wrong. (* WRONG: that's not the list I want!*)
   rewrite <- auto_app_new_sum.
   rewrite H1. rewrite H2. reflexivity.
 - exists []. exists [[]]. exists[]. rewrite app_nil_r. 
   unfoldall. rewrite unit_par. rewrite unit_new. (* 0 = new a 0 *)
   reflexivity.
 - destruct IHP1 as [ns1 [l1 [lr1 H1]]].
   destruct IHP2 as [ns2 [l2 [lr2 H2]]].
   exists (ns1 ++ ns2). exists (l1 ++ l2). exists (lr1 ++ lr2).
   rewrite H1. rewrite H2. rewrite auto_app_new_par. (* new "distributes" over par*)
   rewrite foldpar_mult. rewrite map_distr_app. rewrite map_distr_app.
   reflexivity.
 - destruct IHP as [ns [ls [lr1 H1]]].
   exists []. exists [[]]. exists [P].
   unfold map. rewrite <- sig_triv_0. rewrite <- app_comm_cons.
   unfold foldpar. rewrite foldr_one. rewrite comm_par. rewrite unit_par.
   rewrite app_nil_l. unfold foldr. rewrite unit_par. apply auto_gen_new.
 - destruct IHP as [ns [l1 [lr1 H]]].
   exists (l ++ ns). exists l1. exists lr1. rewrite H. apply app_new.
Qed.
...
Inductive reaction : proc -> proc -> Prop :=
 ...
 | REACT (a x y: name) (P Q M N: proc) (Ms Ns : sigmands) (*(Heq : a = b)*)
